...
function App() {
  ...
  useEffect(async () => {
    var isLoad = await loadData();// load data on local storage
  ...
  function saveData() {
    ...
    var data = JSON.stringify(input) + "$" + JSON.stringify(tempInputPos) + "$" + 
      JSON.stringify(answer) + "$" + JSON.stringify(correctness) + "$" +  
      JSON.stringify(stats) + "$" + JSON.stringify(keyState);
  ...
  return (
    <div className="App">
      ...
      {/* Modal Help */}
      <div className="w-100 h-100 bg-black bg-opacity-75 d-flex position-fixed" 
        style={{
      ...
      >
        <div className="bg-dark w-30 m-auto rounded">
          {/* MODAL TOP */}
          <div className="p-3 border-bottom">
            <div className="border-0 d-flex">
              <h3 className="m-auto pb-2 text-center">HOW TO PLAY</h3>
              <button type="button" onClick={() => setHelpModal(false)} className="btn-close-white m-0 p-0"></button>
            </div>
      ...
      {/* Modal Alert */}
      <div className="bg-white w-30 mt-5 p-2 ms-auto me-auto rounded text-black text-center start-0 end-0 position-absolute" 
        style={{
      ...
      {/* TOP NAV */}
      <div className="row vh-7 ms-0 border-bottom border-2 border-dark d-flex">
        <div className="col h-100 p-0 text-start d-flex">
          <button type="button" className="btn h-100 my-auto mx-2 p-0 text-right text-white" onClick={()=> window.open("https://github.com/mrtanatorn44/wordle-clone", "_blank")}>
          ...
          <button type="button" onClick={() => setHelpModal(true)} className="btn my-auto mx-2 p-0 text-right text-white h-100">
        ...
        <div className="col h-100 p-0 d-flex">
          <h1 className="m-auto text-center fw-bold">Wordle</h1>
        </div>

        <div className="col h-100 p-0 d-flex">
          <p className="m-auto me-2 text-end mobile-hide">by Tanatorn Boonprasert</p>
          <button type="button" className="btn my-auto mx-2 p-0 text-right text-white h-100" onClick={() => setHelpModal(true)}>
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="currentColor" className="bi bi-question-circle" viewBox="0 0 16 16">
      ...
      {/* CONTENT */}
      <div className="vh-63 py-5 text-white">
      ...
      {/* ON-SCREEN KEYBOARD */}
		  <div className="vh-30 py-4 text-white">
        { keyboard.map((rowKey, row) =>
          <div className='row m-auto p-0 justify-content-center' key={row}>
